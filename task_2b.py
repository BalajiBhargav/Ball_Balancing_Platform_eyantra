...
def get_vision_sensor_image(rec_client_id, cam_handle):
    """
    Purpose:
    ---
    ...
    ##############	ADD YOUR CODE HERE	##############
    sim.simxGetPingTime(rec_client_id)

    return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(
        rec_client_id, cam_handle, 0, sim.simx_opmode_oneshot_wait)
...
def send_data(client, maze_array1, maze_array2, maze_array3, maze_array4):
    """
    Purpose:
    ---
    ...
    for i in range(10):
        return_code, Ints, Floats, Strings, Buffer = sim.simxCallScriptFunction(
            client_id, 'top_plate_respondable_t4_1', sim.sim_scripttype_customizationscript, 'receiveData', maze_array1[i], [], [], emptybuff, sim.simx_opmode_blocking)
    for i in range(10):
        return_code, Ints, Floats, Strings, Buffer = sim.simxCallScriptFunction(
            client_id, 'top_plate_respondable_t1_1', sim.sim_scripttype_customizationscript, 'receiveData', maze_array2[i], [], [], emptybuff, sim.simx_opmode_blocking)
    for i in range(10):
        return_code, Ints, Floats, Strings, Buffer = sim.simxCallScriptFunction(
            client_id, 'top_plate_respondable_t2_1', sim.sim_scripttype_customizationscript, 'receiveData', maze_array3[i], [], [], emptybuff, sim.simx_opmode_blocking)
    for i in range(10):
        return_code, Ints, Floats, Strings, Buffer = sim.simxCallScriptFunction(
            client_id, 'top_plate_respondable_t3_1', sim.sim_scripttype_customizationscript, 'receiveData', maze_array4[i], [], [], emptybuff, sim.simx_opmode_blocking)
